...
PATTERNS = [
    ...
    },
    {
        ...
        "post_hash": "#FX #副業",
        "reply": (
            "【口座名】で入金ボーナスキャンペーン中です。\n\n"
            "入金額に応じてボーナスが上乗せされる仕組みで、\n"
    ...
    },
    {
        ...
        "post_hash": "#FX #FX初心者",
        "reply": (
            "【口座名】の入金ボーナスキャンペーンを使いました。\n\n"
            "〇万円入金したらボーナス〇万円が加算されて、\n"
            "実際に動かせる証拠金が〇万円になった感じです。\n\n"
            "ボーナス分はそのままトレードに使えるので\n"
            "実質的に資金効率が上がっています。\n\n"
            "「同じ入金額でも証拠金が多い状態で始められる」\n"
            "これ、特に今の相場では差が出ると思います。\n\n"
            "まだ口座を持っていない方は\n"
            "新規開設 + 入金でダブルボーナスが取れます👇\n"
            "▶️ [URL]\n\n"
            "期間限定なので早めに。"
        ),
    },
...
class PDF:
    def __init__(self, path):
        ...
        self.c.setTitle("入金ボーナスキャンペーン訴求 投稿セット")
        self.page = 0
        self.y    = H - MT
        self._bg()
    ...
    def cover(self):
        ...
        c.drawCentredString(W/2, H - 56*mm,
            "入金額アップ × 新規獲得 両取り設計  |  2026年5月 トレンドワード使用")
    ...
    def guide_page(self):
        ...
        c.drawString(ML + 6*mm, self.y - 5.5*mm, "【口座名】と【URL】・キャンペーン詳細数字を実際の情報に差し替えてご使用ください")
        c.setFillColor(NAVY); c.setFont("JA", 7.5)
        notes = [
            "・ボーナス率（例：入金額の100%）や上限金額は口座ごとに異なります",
            "・キャンペーン期間は確認のうえ〇日まで等の具体的な日付を入れると効果が上がります",
        ]
        ny = self.y - 11*mm
        for n in notes:
